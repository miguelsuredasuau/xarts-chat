...
/** Shoelace area of a simple polygon given as [x, y] points. */
...
/** Area shared by two convex polygons (Sutherland–Hodgman clip of `a` by `b`). */
export function polygonIntersectionArea(a, b) {
  const orient = polygonSignedArea(b) >= 0 ? 1 : -1;
  let out = a;
  for (let i = 0; i < b.length && out.length; i++) {
    const [ex1, ey1] = b[i]; const [ex2, ey2] = b[(i + 1) % b.length];
    const inside = ([x, y]) => orient * ((ex2 - ex1) * (y - ey1) - (ey2 - ey1) * (x - ex1)) >= 0;
    const cross = ([x1, y1], [x2, y2]) => {
      const d = (x1 - x2) * (ey1 - ey2) - (y1 - y2) * (ex1 - ex2);
      if (d === 0) return [x1, y1];
      const t = ((x1 - ex1) * (ey1 - ey2) - (y1 - ey1) * (ex1 - ex2)) / d;
  ...
  return out.length < 3 ? 0 : polygonArea(out);
}
...
export async function inspectSvg(svg, { scale = 1.5 } = {}) {
  ...
  try {
    ...
    const measured = await page.evaluate(() => {
      ...
      // Rotated labels are measured by their real (oriented) box, not the axis-aligned
      // envelope a rotated run would otherwise sweep over its neighbours.
      const quadOf = n => {
        const bb = n.getBBox(); const m = n.getScreenCTM();
        if (!m) return null;
        const pt = (x, y) => [m.a * x + m.c * y + m.e - R.left, m.b * x + m.d * y + m.f - R.top];
        return [pt(bb.x, bb.y), pt(bb.x + bb.width, bb.y), pt(bb.x + bb.width, bb.y + bb.height), pt(bb.x, bb.y + bb.height)];
      };
      const texts = nodes.map((n, i) => {
        const r = n.getBoundingClientRect();
        return { i, text: n.textContent.replace(/\s+/g, ' ').trim(), x: r.left - R.left, y: r.top - R.top, w: r.width, h: r.height, quad: quadOf(n) };
    ...
    for (let a = 0; a < texts.length; a++) {
      for (let c = a + 1; c < texts.length; c++) {
        ...
        const ratio = p.quad && q.quad
          ? polygonIntersectionArea(p.quad, q.quad) / Math.min(polygonArea(p.quad), polygonArea(q.quad))
          : (ix * iy) / Math.min(p.w * p.h, q.w * q.h);
